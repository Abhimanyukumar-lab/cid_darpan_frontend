...
@Component({
  selector: 'app-view-court-lower',
  templateUrl: './view-court-lower.component.html',
  styleUrls: ['./view-court-lower.component.scss'],
})
export class ViewCourtLowerComponent implements OnInit, OnDestroy {
  subscription: any;
  baseUrl: string = AppConstants.backServer;
  lowerCourt: any;

  courtDetailsPath: string =
    AppConstants.LOWER_COURT_MODULE.FETCH_COURT_DETAILS_LIST;
  ...
  changeStatusPath: string = AppConstants.LOWER_COURT_MODULE.FETCH_VIEW_URL;
  ...
  forwardDestinationPath: string =
    AppConstants.LOWER_COURT_MODULE.FETCH_VIEW_FORWARDURL;
  ...
  constructor(
    ...
    this.lowerCourt = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.courtDetailsId = this.lowerCourt.id;
    this.changeStatusId = this.lowerCourt.id;
    this.forwardDestinationId = this.lowerCourt.id;

    this.changeStatusSubmitURL =
      AppConstants.LOWER_COURT_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.LOWER_COURT_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.LOWER_COURT_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.LOWER_COURT_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.LOWER_COURT_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.LOWER_COURT_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.LOWER_COURT_MODULE.DETELE_FORWARD_URL;

    this.permissions.courtDetailsForm = this.global.checkForUserButtonPermission(
      AppConstants.LOWER_COURT_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.courtDetailsList = this.global.checkForUserButtonPermission(
      AppConstants.LOWER_COURT_MODULE.COURT_DETAILS_TABLE
    );
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.LOWER_COURT_MODULE.FETCH_VIEW_DATA,
        { id: this.lowerCourt.id },
        true
      )
      .subscribe((data) => {
        this.lowerCourt = data.courtDTO;

        this.isComplete =
          AppConstants.LOWER_COURT_MODULE.COPM_CLOSED != this.lowerCourt.status
            ? AppConstants.LOWER_COURT_MODULE.COPM_REJECT !=
              this.lowerCourt.status
            : false;
      });
  };
}
