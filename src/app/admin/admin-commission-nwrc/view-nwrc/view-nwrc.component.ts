...
@Component({
  selector: 'app-view-nwrc',
  templateUrl: './view-nwrc.component.html',
  styleUrls: ['./view-nwrc.component.scss'],
})
export class ViewNwrcComponent implements OnInit, OnDestroy {
  subscription: any;
  baseUrl: string = AppConstants.backServer;
  nwrc: any;

  changeStatusPath: string = AppConstants.NWRC_MODULE.FETCH_VIEW_URL;
  ...
  forwardDestinationPath: string =
    AppConstants.NWRC_MODULE.FETCH_VIEW_FORWARDURL;
  ...
  constructor(
    ...
    this.nwrc = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.changeStatusId = this.nwrc.id;
    this.forwardDestinationId = this.nwrc.id;

    this.changeStatusSubmitURL = AppConstants.NWRC_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.NWRC_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.NWRC_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.NWRC_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.NWRC_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.NWRC_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.NWRC_MODULE.DETELE_FORWARD_URL;

    this.permissions.changeStatusForm = this.global.checkForUserButtonPermission(
      AppConstants.NWRC_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.asssignToOfficer = this.global.checkForUserButtonPermission(
      AppConstants.NWRC_MODULE.ASSIGN_TO_OFFICER_FORM
    );

    this.permissions.changeStatusList = this.global.checkForUserButtonPermission(
      AppConstants.NWRC_MODULE.CHANGE_STATUS_TABLE
    );

    this.permissions.forwardToDestinationList = this.global.checkForUserButtonPermission(
      AppConstants.NWRC_MODULE.FORWARD_TO_OFFICER_TABLE
    );

    this.permissions.forwardToDestination = this.global.checkForUserButtonPermission(
      AppConstants.NWRC_MODULE.FORWARD_TO_OFFICER_FORM
    );
  }
  ngOnDestroy(): void {
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.NWRC_MODULE.FETCH_VIEW_DATA,
        { id: this.nwrc.id },
        true
      )
      .subscribe((data) => {
        this.nwrc = data.commissionDTO;

        this.isComplete =
          AppConstants.NWRC_MODULE.COPM_CLOSED != this.nwrc.status
            ? AppConstants.NWRC_MODULE.COPM_REJECT != this.nwrc.status
            : false;
      });
  };
