...
@Component({
  selector: 'app-view-found-person',
  templateUrl: './view-found-person.component.html',
  styleUrls: ['./view-found-person.component.scss'],
})
export class ViewFoundPersonComponent implements OnInit, OnDestroy {
  subscription: any;
  baseUrl: string = AppConstants.backServer;
  foundPerson: any;

  changeStatusPath: string = AppConstants.FOUND_PERSON_MODULE.FETCH_VIEW_URL;
  changeStatusDeleteCode: string;
  changeStatusDeleteUrl: string;
  changeStatusModule: string = 'FOUNDPERSON';
  changeStatusId: number = null;

  forwardDestinationPath: string =
    AppConstants.FOUND_PERSON_MODULE.FETCH_VIEW_FORWARDURL;
  forwardDestinationDeleteCode: string;
  forwardDestinationDeleteUrl: string;
  forwardDestinationModule: string = 'FOUNDPERSON';
  ...
  changeStatusForModule: string = 'FOUNDPERSON';
  ...
  constructor(
    ...
    this.foundPerson = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.changeStatusId = this.foundPerson.id;
    this.forwardDestinationId = this.foundPerson.id;

    this.changeStatusSubmitURL =
      AppConstants.FOUND_PERSON_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.FOUND_PERSON_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.FOUND_PERSON_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.FOUND_PERSON_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.FOUND_PERSON_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.FOUND_PERSON_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.FOUND_PERSON_MODULE.DETELE_FORWARD_URL;


    this.permissions.changeStatusForm = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.asssignToOfficer = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.ASSIGN_TO_OFFICER_FORM
    );

    this.permissions.changeStatusList = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.CHANGE_STATUS_TABLE
    );

    this.permissions.forwardToDestinationList = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.FORWARD_TO_OFFICER_TABLE
    );

    this.permissions.forwardToDestination = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.FORWARD_TO_OFFICER_FORM
    );

    this.permissions.sendSMSToUser = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.SMS_TO_USER_FORM
    );

    this.permissions.sendSMSToUserList = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.SMS_TO_USER_TABLE
    );

    this.permissions.activeInactive = this.global.checkForUserButtonPermission(
      AppConstants.FOUND_PERSON_MODULE.ACTIVE_BUTTON
    );
  }
  ngOnDestroy(): void {
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.FOUND_PERSON_MODULE.FETCH_VIEW_DATA,
        { id: this.foundPerson.id },
        true
      )
      .subscribe((data) => {
        this.foundPerson = data.foundPersonDTO;

        this.moduleStatus = this.foundPerson.status;

        this.isComplete =
          AppConstants.FOUND_PERSON_MODULE.COPM_CLOSED !=
          this.foundPerson.status
            ? AppConstants.FOUND_PERSON_MODULE.COPM_REJECT !=
              this.foundPerson.status
            : false;
            
        this.isPending = this.foundPerson.status == AppConstants.PENDING;
      });
  };
  ...
  acceptApp = (acceptReject: number, id: string) => {
    this.apiCaller
      .apiPostCall(
        AppConstants.FOUND_PERSON_MODULE.SHO_HIDE_PUBLICE,
        {
          id: this.foundPerson.id,
