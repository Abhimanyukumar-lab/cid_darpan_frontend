...
@Component({
  selector: 'app-view-shrc',
  templateUrl: './view-shrc.component.html',
  styleUrls: ['./view-shrc.component.scss'],
})
export class ViewShrcComponent implements OnInit, OnDestroy {
  subscription: any;
  baseUrl: string = AppConstants.backServer;
  shrc: any;

  changeStatusPath: string = AppConstants.SHRC_MODULE.FETCH_VIEW_URL;
  ...
  forwardDestinationPath: string =
    AppConstants.SHRC_MODULE.FETCH_VIEW_FORWARDURL;
  ...
  constructor(
    ...
    this.shrc = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.changeStatusId = this.shrc.id;
    this.forwardDestinationId = this.shrc.id;

    this.changeStatusSubmitURL = AppConstants.SHRC_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.SHRC_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.SHRC_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.SHRC_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.SHRC_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.SHRC_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.SHRC_MODULE.DETELE_FORWARD_URL;

    this.permissions.changeStatusForm = this.global.checkForUserButtonPermission(
      AppConstants.SHRC_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.asssignToOfficer = this.global.checkForUserButtonPermission(
      AppConstants.SHRC_MODULE.ASSIGN_TO_OFFICER_FORM
    );

    this.permissions.changeStatusList = this.global.checkForUserButtonPermission(
      AppConstants.SHRC_MODULE.CHANGE_STATUS_TABLE
    );

    this.permissions.forwardToDestinationList = this.global.checkForUserButtonPermission(
      AppConstants.SHRC_MODULE.FORWARD_TO_OFFICER_TABLE
    );

    this.permissions.forwardToDestination = this.global.checkForUserButtonPermission(
      AppConstants.SHRC_MODULE.FORWARD_TO_OFFICER_FORM
    );
  }
  ngOnDestroy(): void {
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.SHRC_MODULE.FETCH_VIEW_DATA,
        { id: this.shrc.id },
        true
      )
      .subscribe((data) => {
        this.shrc = data.commissionDTO;

        this.isComplete =
          AppConstants.SHRC_MODULE.COPM_CLOSED != this.shrc.status
            ? AppConstants.SHRC_MODULE.COPM_REJECT != this.shrc.status
            : false;
      });
  };
