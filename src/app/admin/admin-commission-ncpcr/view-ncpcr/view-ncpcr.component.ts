...
@Component({
  selector: 'app-view-ncpcr',
  templateUrl: './view-ncpcr.component.html',
  styleUrls: ['./view-ncpcr.component.scss']
})
export class ViewNcpcrComponent implements OnInit, OnDestroy {
  subscription: any;
  baseUrl: string = AppConstants.backServer;
  ncpcr: any;

  changeStatusPath: string = AppConstants.NCPCR_MODULE.FETCH_VIEW_URL;
  ...
  forwardDestinationPath: string =
    AppConstants.NCPCR_MODULE.FETCH_VIEW_FORWARDURL;
  ...
  constructor(
    ...
    this.ncpcr = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.changeStatusId = this.ncpcr.id;
    this.forwardDestinationId = this.ncpcr.id;

    this.changeStatusSubmitURL = AppConstants.NCPCR_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.NCPCR_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.NCPCR_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.NCPCR_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.NCPCR_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.NCPCR_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.NCPCR_MODULE.DETELE_FORWARD_URL;

    this.permissions.changeStatusForm = this.global.checkForUserButtonPermission(
      AppConstants.NCPCR_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.asssignToOfficer = this.global.checkForUserButtonPermission(
      AppConstants.NCPCR_MODULE.ASSIGN_TO_OFFICER_FORM
    );

    this.permissions.changeStatusList = this.global.checkForUserButtonPermission(
      AppConstants.NCPCR_MODULE.CHANGE_STATUS_TABLE
    );

    this.permissions.forwardToDestinationList = this.global.checkForUserButtonPermission(
      AppConstants.NCPCR_MODULE.FORWARD_TO_OFFICER_TABLE
    );

    this.permissions.forwardToDestination = this.global.checkForUserButtonPermission(
      AppConstants.NCPCR_MODULE.FORWARD_TO_OFFICER_FORM
    );
  }
  ngOnDestroy(): void {
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.NCPCR_MODULE.FETCH_VIEW_DATA,
        { id: this.ncpcr.id },
        true
      )
      .subscribe((data) => {
        this.ncpcr = data.commissionDTO;

        this.isComplete =
          AppConstants.NCPCR_MODULE.COPM_CLOSED != this.ncpcr.status
            ? AppConstants.NCPCR_MODULE.COPM_REJECT != this.ncpcr.status
            : false;
      });
  };
