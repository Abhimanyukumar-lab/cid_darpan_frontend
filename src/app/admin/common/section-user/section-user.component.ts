...
export class SectionUserComponent implements OnInit, OnDestroy {
  subscription: any;
  sectionsUserPage: Page;
  sectionsUserRows = new Array<StationUser>();
  ...
  @Input()
  sectionsUserPath: string;

  @Input()
  sectionsUserDeleteCode: string;

  @Input()
  sectionsUserDeleteUrl: string;

  @Input()
  sectionsUserId: number;

  @Input()
  sectionsUserEditCurrentFormCode: string;
  ...
  sectionsUserPermissions: Permissions = new Permissions();
  ...
  ngOnInit(): void {
    this.sectionsUserPermissions.delete =
      this.global.checkForUserButtonPermission(this.sectionsUserDeleteCode);
    this.sectionsUserPermissions.edit_current_form =
      this.global.checkForUserButtonPermission(
        this.sectionsUserEditCurrentFormCode
      );

    this.sectionsUserPermissions.delete_url = this.sectionsUserDeleteUrl;

    this.sectionsUserPage = new Page(
      0,
      0,
      0,
      0,
    ...
    this.subscription = this.appStore.pipe(select('app')).subscribe((data) => {
      if (data.isEditing) {
        ...
        this.STATION_USER_PARAM.sectionsId = this.sectionsUserId;
        this.initStationUserForm();
      }
    });
  }
  ...
  initStationUserForm = () => {
    this.form = this.fb.group({
      ...
      sectionsId: [
        this.STATION_USER_PARAM.sectionsId,
  ...
  resetForm = () => {
    ...
    this.STATION_USER_PARAM.sectionsId = null;
    this.STATION_USER_PARAM.sectionsName = null;
    this.STATION_USER_PARAM.sectionsNameHi = null;
