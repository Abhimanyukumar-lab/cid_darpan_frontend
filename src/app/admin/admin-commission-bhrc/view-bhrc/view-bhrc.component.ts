...
@Component({
  selector: 'app-view-bhrc',
  templateUrl: './view-bhrc.component.html',
  styleUrls: ['./view-bhrc.component.scss']
})
export class ViewBhrcComponent implements OnInit, OnDestroy {
  subscription: any;

  baseUrl: string = AppConstants.backServer;
  bhrc: any;

  changeStatusPath: string = AppConstants.BHRC_MODULE.FETCH_VIEW_URL;
  ...
  forwardDestinationPath: string =
    AppConstants.BHRC_MODULE.FETCH_VIEW_FORWARDURL;
  ...
  constructor(
    ...
    this.bhrc = this.localStorage.getStoredValue('viewData');

    this.fetchData();

    this.changeStatusId = this.bhrc.id;
    this.forwardDestinationId = this.bhrc.id;

    this.changeStatusSubmitURL = AppConstants.BHRC_MODULE.CHANGE_STATUS_SUBMIT;
    this.assignToOfficerSubmitURL =
      AppConstants.BHRC_MODULE.ASSIGN_TO_OFFICER_SUBMIT;
    this.forwatdToDestinationSubmitURL =
      AppConstants.BHRC_MODULE.FORWARD_TO_DESTINATION_SUBMIT;

    this.changeStatusDeleteCode =
      AppConstants.BHRC_MODULE.DELETE_CHECK_STATUS_BUTTON;
    this.changeStatusDeleteUrl =
      AppConstants.BHRC_MODULE.DETELE_CHECK_STATUS_URL;

    this.forwardDestinationDeleteCode =
      AppConstants.BHRC_MODULE.DELETE_FORWARD_TO_BUTTON;
    this.forwardDestinationDeleteUrl =
      AppConstants.BHRC_MODULE.DETELE_FORWARD_URL;

    this.permissions.changeStatusForm = this.global.checkForUserButtonPermission(
      AppConstants.BHRC_MODULE.CHANGE_STATUS_FORM
    );

    this.permissions.asssignToOfficer = this.global.checkForUserButtonPermission(
      AppConstants.BHRC_MODULE.ASSIGN_TO_OFFICER_FORM
    );

    this.permissions.changeStatusList = this.global.checkForUserButtonPermission(
      AppConstants.BHRC_MODULE.CHANGE_STATUS_TABLE
    );

    this.permissions.forwardToDestinationList = this.global.checkForUserButtonPermission(
      AppConstants.BHRC_MODULE.FORWARD_TO_OFFICER_TABLE
    );

    this.permissions.forwardToDestination = this.global.checkForUserButtonPermission(
      AppConstants.BHRC_MODULE.FORWARD_TO_OFFICER_FORM
    );
  }
  ngOnDestroy(): void {
  ...
  fetchData = () => {
    this.apiCaller
      .apiPostCall(
        AppConstants.BHRC_MODULE.FETCH_VIEW_DATA,
        { id: this.bhrc.id },
        true
      )
      .subscribe((data) => {
        this.bhrc = data.commissionDTO;

        this.isComplete =
          AppConstants.BHRC_MODULE.COPM_CLOSED != this.bhrc.status
            ? AppConstants.BHRC_MODULE.COPM_REJECT != this.bhrc.status
            : false;
      });
  };
